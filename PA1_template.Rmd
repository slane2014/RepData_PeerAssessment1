...
For this section, we summed the number of steps taken within each 24-hour period
within each day.

```{r histogram}
steps_per_day <- ddply(data, .(date_posix), summarize, sum = sum(steps))
hist1 <- hist(steps_per_day$sum, breaks = 10)
plot(hist1,
     main = "Histogram of total steps taken within each 24-hour period",
     xlab = "Total steps")
```

As shown in the graph above, the 24-hour periods that contained between 10,000 and 12,000 total
steps were most frequently observed. For example, Days 3 and 4 (`r steps_per_day$date_posix[3]` and
`r steps_per_day$date_posix[4]`) contained `r steps_per_day$sum[3]` and `r steps_per_day$sum[4]` 
total steps each, respectively. The results from these two days contributed to the greatest
...
As shown in the graph above, the mean number of steps taken per day varies betwen approximately 
`r min_steps` and `r max_steps`. For example, the mean number of steps taken during Day 2 (`r steps_metrics$date_posix[2]`) across all 5-minutes intervals was `r steps_metrics$mean[2]`. (Values
of NA are not plotted.)
...
across all days. This represents the 5-minute interval starting at `r time` each
...
For this task, I replaced each NA value with the rounded mean for the corresponding 5-minute interval. If the rounded mean for the 5-min interval is NA, then I replaced it with '0'.
...
```{r mod_histogram,results='hide',message=FALSE,include=FALSE}
...
plot(mod_steps_metrics$mean,
     main = "Average number of total steps (modified data) across each 24-hour period",
...
xyplot(steps ~ interval | day_of_week_mod, 
        ...
        layout = c(1, 2), 
        type = "l", 
        main = "Mean number of total steps across each 5-minutes interval, split
